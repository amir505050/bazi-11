...
while True:
    if start_menu:
        ...
        if har == 1 and turn:
            ...
            client.send(f"2{list_of_cart[sel-1]}{sel_h-1}".encode())
            ...
            client.send(f"7{s_n-1} ".encode())
            print("please waiting for seeking ...")
            j_s = client.recv(1024).decode()
            print("Succsesful")
            ...
            if data[0] == "7":
                ss = int(data[1])
                client.send(f"{list_of_cart[ss]}".encode())
                print("-------Seeking-------")
                time.sleep(3)
                turn = True
            if data[0] == "8":
